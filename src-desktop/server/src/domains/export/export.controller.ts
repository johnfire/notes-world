...
export const exportUntagged = wrapAsync(
...
export const exportJson = wrapAsync(async (_req: Request, res: Response) => {
  const data = await service.exportJson(PHASE1_USER_ID);
  const date = new Date().toISOString().slice(0, 10);
...
export const exportCsv = wrapAsync(async (_req: Request, res: Response) => {
  const csv = await service.exportCsv(PHASE1_USER_ID);
  const date = new Date().toISOString().slice(0, 10);
