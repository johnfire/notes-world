...
export async function updateImportJobStatus(
  ...
  if (updates.items_found !== undefined) {
    sets.push(`items_found = $${idx++}`);
    values.push(updates.items_found);
  }
  if (updates.items_imported !== undefined) {
    sets.push(`items_imported = $${idx++}`);
    values.push(updates.items_imported);
  }
  if (updates.items_skipped !== undefined) {
    sets.push(`items_skipped = $${idx++}`);
    values.push(updates.items_skipped);
  }
  if (updates.items_failed !== undefined) {
    sets.push(`items_failed = $${idx++}`);
  ...
  values.push(id);
  await getPool().query(
    `UPDATE import_jobs SET ${sets.join(", ")} WHERE id = $${idx}`,
    values,
  );
}
...
export async function findImportRecords(
  importJobId: ImportJobId,
  userId: UserId,
): Promise<ImportRecord[]> {
  const { rows } = await getPool().query<ImportRecord>(
    `SELECT ir.* FROM import_records ir
     JOIN import_jobs ij ON ij.id = ir.import_job_id
     WHERE ir.import_job_id = $1 AND ij.user_id = $2
