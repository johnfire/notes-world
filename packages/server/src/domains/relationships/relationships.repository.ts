...
import { Tag, TagId, UserId, ItemId, TagWithCount } from "../../types";
import { query, queryOne, getPool } from "../../db/client";
import { TagWithCount as InternalTagWithCount } from "./relationships.types";

export async function findTagById(
  id: TagId,
  userId: UserId,
): Promise<Tag | null> {
  return queryOne<Tag>("SELECT * FROM tags WHERE id = $1 AND user_id = $2", [
    id,
    userId,
  ]);
}
...
export async function findTagByName(
  ...
  return queryOne<Tag>("SELECT * FROM tags WHERE name = $1 AND user_id = $2", [
    name,
    userId,
  ]);
}
...
export async function findTagsForItem(
  itemId: ItemId,
  userId: UserId,
): Promise<Tag[]> {
  return query<Tag>(
...
export async function itemTagExists(
  itemId: ItemId,
  tagId: TagId,
): Promise<boolean> {
...
export async function deleteItemTag(
  ...
  await query("DELETE FROM item_tags WHERE item_id = $1 AND tag_id = $2", [
    itemId,
    tagId,
  ]);
}

export async function findTagsForItems(
  itemIds: ItemId[],
  userId: UserId,
