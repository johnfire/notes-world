...
export async function findDashboardByUser(
  userId: UserId,
): Promise<Dashboard | null> {
  return queryOne<Dashboard>("SELECT * FROM dashboards WHERE user_id = $1", [
    userId,
  ]);
}
...
export async function findBlocksByDashboard(
...
export async function countBlocksByDashboard(
  dashboardId: DashboardId,
): Promise<number> {
...
export async function updateBlock(
  ...
  const { sql, params } = buildUpdate(
    ...
    {
      jsonFields: ["config"],
      allowedFields: ["view_type", "title", "row", "col", "config"],
    },
  );
...
export async function deleteBlock(id: BlockId, userId: UserId): Promise<void> {
  await query("DELETE FROM blocks WHERE id = $1 AND user_id = $2", [
    id,
    userId,
  ]);
}
