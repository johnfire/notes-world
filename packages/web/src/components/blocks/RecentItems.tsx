...
function ItemRow({ item, tags }: { item: Item; tags: Tag[] }) {
  ...
  const ago = (() => {
    ...
    if (mins < 1) return t("app.blocks.recentItems.justNow");
    if (mins < 60) return t("app.blocks.recentItems.minsAgo", { mins });
    const hrs = Math.floor(mins / 60);
    if (hrs < 24) return t("app.blocks.recentItems.hrsAgo", { hrs });
    return t("app.blocks.recentItems.daysAgo", { days: Math.floor(hrs / 24) });
  })();

  return (
    <button
      onClick={() => openItem(item.id)}
      className="w-full text-left flex items-start gap-3 py-2 border-b border-surface-500 last:border-0 group cursor-pointer hover:bg-surface-600 -mx-4 px-4 transition-colors"
    >
        ...
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1">
            {tags.map((tag) => (
              <span
                key={tag.id}
                className="text-xs px-1.5 py-0.5 rounded bg-surface-600 text-accent"
              >
                {tag.name}
              </span>
            ))}
    ...
    <div className="card h-full overflow-hidden flex flex-col">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">
          {block.title ?? t("app.blocks.recentItems.title")}
        </h3>
      ...
      <div className="flex-1 overflow-y-auto -mx-4 px-4">
        {items.length === 0 ? (
          <p className="text-sm text-gray-600 py-4 text-center">
            {t("app.blocks.recentItems.empty")}
          </p>
        ) : (
          items.map((item) => (
            <ItemRow key={item.id} item={item} tags={tagMap[item.id] ?? []} />
          ))
        )}
      </div>
    </div>
