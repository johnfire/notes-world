...
describe("buildUpdate", () => {
  ...
  // Audit LOW #11: column/table names are interpolated, so a malicious
  // identifier must be rejected rather than becoming injectable SQL.
  test("throws on an injectable column identifier", () => {
    expect(() =>
      buildUpdate(
        "items",
        { "title = 'x', user_id = 'victim' --": 1 },
        { id: "x" },
      ),
    ).toThrow(/unsafe SQL identifier/);
  });

  test("throws on an injectable table name", () => {
    expect(() =>
      buildUpdate("items; DROP TABLE users", { title: "x" }, { id: "x" }),
    ).toThrow(/unsafe SQL identifier/);
  });

  test("throws on an injectable where-clause identifier", () => {
    expect(() =>
      buildUpdate("items", { title: "x" }, { "id = id OR 1=1 --": "x" }),
    ).toThrow(/unsafe SQL identifier/);
  });
});
