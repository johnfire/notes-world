...
describe("createTag", () => {
  ...
  test("throws ValidationError when name is empty", async () => {
    await expect(service.createTag(TEST_USER_ID, "")).rejects.toMatchObject({
  ...
  test("throws ValidationError when name exceeds limit", async () => {
    ...
    mockRepo.findTagByName.mockResolvedValue(null);
    await expect(
      service.createTag(TEST_USER_ID, longName),
  ...
  test("throws ConflictError when tag already exists", async () => {
    ...
    await expect(service.createTag(TEST_USER_ID, "work")).rejects.toMatchObject(
      { code: "CONFLICT_ERROR" },
    );
    expect(mockRepo.insertTag).not.toHaveBeenCalled();
  });
...
describe("renameTag", () => {
  test("renames tag and returns updated", async () => {
    ...
    expect(mockRepo.updateTagName).toHaveBeenCalledWith(
      tag.id,
      TEST_USER_ID,
      "new",
    );
  ...
  test("throws NotFoundError when tag missing", async () => {
    ...
    await expect(
      service.renameTag(TEST_USER_ID, "x", "new"),
  ...
  test("throws ValidationError when new name is empty", async () => {
    ...
    await expect(
      service.renameTag(TEST_USER_ID, "x", ""),
    ).rejects.toMatchObject({ code: "VALIDATION_ERROR" });
  });
  ...
  test("throws ConflictError when new name is taken by different tag", async () => {
    ...
    await expect(
      service.renameTag(TEST_USER_ID, tag.id, "taken"),
  ...
  test("allows rename to same name (no conflict when same tag id)", async () => {
    ...
    await expect(service.renameTag(TEST_USER_ID, tag.id, "same")).resolves.toBe(
      updated,
    );
  });
});
...
describe("deleteTag", () => {
  ...
  test("throws NotFoundError when tag missing", async () => {
    ...
    await expect(service.deleteTag(TEST_USER_ID, "x")).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
  });
...
describe("getAllTags", () => {
  test("returns all tags for user", async () => {
    const tags = [makeTagWithCount(), makeTagWithCount()];
...
describe("getTagsForItem", () => {
  test("returns tags for item", async () => {
    ...
    expect(result).toBe(tags);
    expect(mockRepo.findTagsForItem).toHaveBeenCalledWith(
      "item-id-1",
      TEST_USER_ID,
    );
  });
...
describe("tagItem", () => {
  test("tags item and emits ItemTagged", async () => {
    ...
    expect(mockRepo.insertItemTag).toHaveBeenCalledWith(
      item.id,
      tag.id,
      TEST_USER_ID,
    );
  ...
  test("throws NotFoundError when item missing", async () => {
    ...
    await expect(service.tagItem(TEST_USER_ID, "x", "y")).rejects.toMatchObject(
      { code: "NOT_FOUND" },
    );
  });
  ...
  test("throws AuthorizationError when item not owned by user", async () => {
    mockItemRepo.findById.mockResolvedValue(
      makeItem({ user_id: OTHER_USER_ID }),
    );
...
describe("untagItem", () => {
  ...
  test("throws NotFoundError when item missing", async () => {
    ...
    await expect(
      service.untagItem(TEST_USER_ID, "x", "y"),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
  });
...
describe("getItemsForTag", () => {
  test("returns items for tag with default pagination", async () => {
    ...
    expect(mockItemRepo.findByTag).toHaveBeenCalledWith(
      TEST_USER_ID,
      "tag-id-1",
      50,
      0,
...
describe("findOrCreateTag", () => {
  ...
  test("normalizes name before lookup", async () => {
    ...
    expect(mockRepo.findTagByName).toHaveBeenCalledWith(
      "normalized",
      TEST_USER_ID,
    );
  });
