...
jest.mock(
  "../../../../../packages/server/src/domains/relationships/relationships.service",
);
...
describe("GET /api/tags", () => {
  test("200 with all tags", async () => {
    const tags = [
      makeTagWithCount({ name: "work" }),
      makeTagWithCount({ name: "personal" }),
    ];
...
describe("POST /api/tags", () => {
  ...
  test("422 when name is empty", async () => {
    ...
    mockService.createTag.mockRejectedValue(
      new ValidationError("Tag name is required"),
    );
  ...
  test("409 when tag already exists", async () => {
    const { ConflictError } =
      await import("../../../../../packages/server/src/utils/errors");
    mockService.createTag.mockRejectedValue(
      new ConflictError('Tag "work" already exists'),
    );
...
describe("PATCH /api/tags/:id", () => {
  test("200 with renamed tag", async () => {
    ...
    expect(mockService.renameTag).toHaveBeenCalledWith(
      TEST_USER_ID,
      tag.id,
      "renamed",
    );
  ...
  test("404 when tag not found", async () => {
    const { NotFoundError } =
      await import("../../../../../packages/server/src/utils/errors");
    mockService.renameTag.mockRejectedValue(
      new NotFoundError("Tag", "missing-id"),
    );
  ...
  test("409 when new name conflicts", async () => {
    const { ConflictError } =
      await import("../../../../../packages/server/src/utils/errors");
    mockService.renameTag.mockRejectedValue(
      new ConflictError("Tag already exists"),
    );
...
describe("GET /api/tags/:id/items", () => {
  test("200 with items for tag", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body).toHaveLength(2);
    expect(mockService.getItemsForTag).toHaveBeenCalledWith(
      TEST_USER_ID,
      "tag-id-1",
      50,
      0,
...
describe("GET /api/tags/item/:itemId", () => {
  test("200 with tags for item", async () => {
    ...
    expect(mockService.getTagsForItem).toHaveBeenCalledWith(
      TEST_USER_ID,
      "item-id-1",
    );
  });
...
describe("POST /api/tags/item/:itemId/:tagId", () => {
  test("204 on successful tag", async () => {
    ...
    expect(res.status).toBe(204);
    expect(mockService.tagItem).toHaveBeenCalledWith(
      TEST_USER_ID,
      "item-id-1",
      "tag-id-1",
    );
  ...
  test("422 when tag limit exceeded", async () => {
    const { LimitExceeded } =
      await import("../../../../../packages/server/src/utils/errors");
    mockService.tagItem.mockRejectedValue(
      new LimitExceeded("Too many tags", { current_count: 20, maximum: 20 }),
    );
...
describe("DELETE /api/tags/item/:itemId/:tagId", () => {
  test("204 on successful untag", async () => {
    ...
    expect(res.status).toBe(204);
    expect(mockService.untagItem).toHaveBeenCalledWith(
      TEST_USER_ID,
      "item-id-1",
      "tag-id-1",
    );
  ...
  test("404 when item not found", async () => {
    const { NotFoundError } =
      await import("../../../../../packages/server/src/utils/errors");
    mockService.untagItem.mockRejectedValue(
      new NotFoundError("Item", "missing"),
    );
